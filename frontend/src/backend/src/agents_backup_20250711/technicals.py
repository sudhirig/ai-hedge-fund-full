...
def technical_analyst_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        def safe_confidence(conf_value):
            """Safely convert confidence to integer, handling NaN values."""
            import math
            if conf_value is None or math.isnan(conf_value):
                return 50  # Default confidence when calculation fails
            return round(max(0, min(100, conf_value * 100)))
